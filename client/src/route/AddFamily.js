...
export const HouseholdForm = () => {
    ...
    const fieldElements = Object.entries(fields).map(([fieldName, fieldValue]) => (
        <TextField
            key={fieldName}
            label={fieldName}
    ...
    const Form = ()=>{
        return(
            <div>
                ...
                <div>
                    <Button variant="contained" sx={{mr:4, mt:4, mb:4}} color='success'>Lưu</Button>
                </div>
                ...
                <div>
                    <Button variant="outlined" sx={{mr:4, mb:4  }} onClick={()=>handleGoBack()}>Trở về</Button>
                </div>
            </div>
        )
    };
